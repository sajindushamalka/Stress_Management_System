...
const ExpensesSection = () => {
  ...
  const renderAlert = (cat, idx) => (
    <View key={idx} style={styles.notificationCard}>
      <View style={styles.iconWrapper}>
        <Icon name="bell" size={18} color="#FF8C00" />
      </View>
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.amountText}>Rs. {weeklyData[cat].totalAmount}</Text>
      </View>
    </View>
  );
    ...
    <LinearGradient colors={["#fff", "#f7f7f7"]} style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {loading ? (
          ...
          Object.keys(weeklyData).map(renderAlert)
        )}
      </ScrollView>
      <Footer />
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  amountText: { fontSize: 14, fontWeight: "700", color: "#FF3B30" },
  notificationCard: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: "#fff",
  ...
  categoryText: { fontSize: 14, fontWeight: "600", color: "#111" },
  transactionText: { fontSize: 12, color: "#666", marginTop: 2 },
});
